...
- Skill local `/mensagem-whatsapp` (`.claude/skills/mensagem-whatsapp/SKILL.md`) gera a mensagem do dia pra colar no WhatsApp a partir de um `.md` de `content/youtube/`, salva em `content/whatsapp/<mesmo-nome>.md`. Links do Instagram (`https://www.instagram.com/prisaiasnog/`) e do site (`https://diego-dm-morais.github.io/a-escada-da-multiplicacao/`) são fixos em toda mensagem. Chamada ao final da pipeline `/pipe-live`.
