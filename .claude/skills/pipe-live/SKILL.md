---
name: pipe-live
description: Pipeline completo — recebe link do YouTube e roda em sequência youtube-to-md, outline, marca-texto, mind-map, jornada-multiplicacao, atualização do index.html e commit+push pra main (dispara publicação automática do site estático). Use quando usuário pedir /pipe-live <URL> ou "processa essa live completa".
---
...
7. `commit + push` — commita tudo e envia pra `main`, disparando o pipeline do GitHub Actions (`.github/workflows/pages.yml`) que publica o site estático em produção
...
7. Ao final, reportar os caminhos/links gerados (youtube, outline já marcado, mind-map do capítulo, link do artifact da jornada atualizado, entrada nova no index.html) e confirmação do push/deploy.
...
- Push só na etapa 6, depois de todas as etapas de conteúdo confirmadas — nunca commitar parcial.
